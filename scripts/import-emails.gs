/**
 * GTA Scrub — Import Scraped Emails to Sheet
 * ============================================
 * Run this AFTER running the email scraper script.
 * Adds an "Import Emails" menu item to the sheet.
 */

const SPREADSHEET_ID = '1-0wOhrEFX5EkiajX0gtNFsVSDCaPObt8rD94kQoK6XA';
const SHEET_NAME = 'Results';

// Email data: [sheetRow, emailString]
const EMAIL_UPDATES = [
...
/**
 * Import emails into column I of the Results sheet.
 * Run from Extensions → Apps Script → Run importEmails
 */
function importEmails() {
  ...
  for (const [rowIndex, email] of EMAIL_UPDATES) {
    const cell = sheet.getRange(rowIndex, 9); // Column I
    const currentValue = cell.getValue();
    // Only write if cell is empty or different
  ...
  SpreadsheetApp.getUi().alert(
    `Emails imported!\nUpdated: ${updated} rows\nSkipped (already has email): ${skipped}`
  );
}

/**
 * Add menu item when sheet opens
 */
function onOpen() {
  const ui = SpreadsheetApp.getUi();
  ui.createMenu('GTA Scrub')
    .addItem('Import Scraped Emails', 'importEmails')
    .addToUi();
}
